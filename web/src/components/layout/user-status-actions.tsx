...
import { BookOpen, Coins, House, Keyboard, Languages, Settings2, UserRound } from "lucide-react";
    ...
    return (
        <div className="inline-flex shrink-0 items-center gap-1">
            {userStatus === "authenticated" ? (
                <PlatformProjectWorkspace compact={compact} className="inline-flex h-7 shrink-0 items-center justify-center gap-1 px-1 text-stone-600 transition hover:text-stone-950 dark:text-stone-300 dark:hover:text-white" style={iconStyle} />
            ) : null}
            ...
            <a href={PLATFORM_HOME_URL} className={cn(naturalIconClass, "!w-auto gap-1.5 px-2")} style={iconStyle} aria-label={t("action.returnPlatformHome")} title={t("action.returnPlatformHome")}>
                <House className="size-4" />
                <span className={cn("text-xs font-medium leading-none", compact && "hidden xl:inline")}>{t("action.returnPlatformHome")}</span>
            </a>
            {ENV_SHOW_DOCS ? (
