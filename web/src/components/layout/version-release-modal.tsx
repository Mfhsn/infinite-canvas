...
function getReleaseTitle(version: string, t: ReturnType<typeof useI18n>["t"]) {
    return version === "Unreleased" ? t("release.unreleased") : version;
}

function getReleaseTypeLabel(type: string, t: ReturnType<typeof useI18n>["t"]) {
    if (type === "新增") return t("release.type.added");
    if (type === "修复") return t("release.type.fixed");
    if (type === "调整") return t("release.type.changed");
    if (type === "文档") return t("release.type.docs");
    return type;
}
            ...
            <Modal title={t("release.title")} open={open} width={680} centered footer={null} onCancel={() => setOpen(false)}>
                <div className="mb-5 grid grid-cols-2 gap-3">
                    <div className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                        <div className="text-xs text-stone-500 dark:text-stone-400">{t("release.currentVersion")}</div>
                        <div className="mt-1 text-base font-semibold text-stone-950 dark:text-stone-100">{APP_VERSION}</div>
                    </div>
                    <div className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                        <div className="flex items-center justify-between gap-3">
                            <div className="text-xs text-stone-500 dark:text-stone-400">{t("release.latestVersion")}</div>
                            <button
                                type="button"
                            ...
                            content: (
                                <div>
                                    <div className="flex flex-wrap items-center gap-2">
                                        <span className="text-sm font-semibold text-stone-950 dark:text-stone-100">{getReleaseTitle(release.version, t)}</span>
                                        <span className="text-xs text-stone-500 dark:text-stone-400">{release.date}</span>
                                        <div className="flex min-w-0 items-center gap-1.5">
                                            {release.version === latestVersion ? <Tag color="green">{t("common.latest")}</Tag> : null}
                                            {release.version === APP_VERSION ? <Tag>{t("common.current")}</Tag> : null}
                                        </div>
                                    </div>
                                        ...
                                        {release.items.map((item, index) => (
                                            <div key={`${release.version}-${index}`} className="flex items-start gap-2 text-sm leading-6 text-stone-700 dark:text-stone-300">
                                                <Tag color={getTagColor(item.type)} className="m-0 mt-0.5 shrink-0 whitespace-nowrap">
                                                    {getReleaseTypeLabel(item.type, t)}
                                                </Tag>
