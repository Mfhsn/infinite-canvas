...
const qualityOptions = [
    ...
    { value: "high", labelKey: "settings.image.quality.high" },
    { value: "medium", labelKey: "settings.image.quality.medium" },
    { value: "low", labelKey: "settings.image.quality.low" },
] as const;
...
export function imageSizeLabel(size: string, t?: ReturnType<typeof useI18n>["t"]) {
    const label = aspectOptions.find((item) => (item.size || item.value) === size || item.value === size)?.label || size;
    return label === "auto" && t ? t("settings.image.quality.auto") : label;
}
