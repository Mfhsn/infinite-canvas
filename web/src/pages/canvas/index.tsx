...
export default function CanvasPage() {
    ...
    const createAndEnter = () => enterProject(createProject(t("canvas.defaultTitle", { index: projects.length + 1 })));
    const importCanvas = async (file?: File) => {
        ...
        try {
            ...
            message.success(t("canvas.importSuccess", { count: data.projects.length }));
        } catch {
    ...
    useEffect(() => {
        ...
        enterProject(mode === "new" ? createProject(t("canvas.defaultTitle", { index: projects.length + 1 })) : projects[0]?.id || createProject(t("canvas.defaultTitle", { index: projects.length + 1 })));
            ...
            <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-10">
                <header className="flex flex-wrap items-end justify-between gap-4 border-b border-stone-200 pb-6 dark:border-stone-800">
                    ...
                    <div className="flex items-center gap-2">
                        {selectedIds.length ? (
                            <>
                                <Button disabled={!hydrated} icon={<Download className="size-4" />} onClick={() => void exportCanvasProjects(projects.filter((project) => selectedIds.includes(project.id)), t("canvas.exportFilename", { count: selectedIds.length }))}>
