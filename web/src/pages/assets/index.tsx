...
export default function AssetsPage() {
    ...
    const saveAsset = async () => {
        ...
        if (values.kind === "text") {
            ...
            if (!imageDraft) {
                message.error(t("assets.imageRequired"));
                return;
            }
    ...
    const exportAllAssets = async () => {
        if (!validAssets.length) {
            message.warning(t("assets.exportEmpty"));
            return;
        }
        await exportAssets(validAssets, t("assets.exportFilename"));
    };
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-background text-stone-900 dark:text-stone-100">
            <main className="min-h-0 flex-1 overflow-y-auto bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] px-6 py-8 [background-size:16px_16px] dark:bg-[radial-gradient(rgba(245,245,244,.14)_1px,transparent_1px)]">
                <div className="pb-8">
                    <div className="mx-auto max-w-5xl text-center">
                        ...
                        <p className="mt-3 text-sm text-stone-500 dark:text-stone-400">{t("assets.subtitle")}</p>
                    </div>
            ...
            <Modal title={editingAsset ? t("assets.editTitle") : t("assets.createTitle")} open={isAssetOpen} width={980} onCancel={() => setIsAssetOpen(false)} onOk={() => void saveAsset()} okText={t("common.save")} cancelText={t("common.cancel")} destroyOnHidden>
            ...
            <button type="button" className="block w-full text-left" onClick={onOpen}>
                <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            ...
                            <Typography.Text type="secondary" className="mt-1 block text-xs">
                                {asset.source || t("assets.unlabeledSource")}
                            </Typography.Text>
                        </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                        ...
                        {!asset.tags?.length ? <Tag className="m-0 text-[11px]">{t("common.noTags")}</Tag> : null}
                    </div>
                </div>
            </button>
...
function AssetDrawer({ asset, onClose, onCopy, onDownload }: { asset: Asset | null; onClose: () => void; onCopy: (asset: Asset) => void; onDownload: (asset: Asset) => void }) {
    ...
    return (
        <Drawer title={t("assets.detailTitle")} open={Boolean(asset)} size="large" onClose={onClose}>
            {asset ? (
...
function assetKindLabel(kind: AssetKind, t: ReturnType<typeof useI18n>["t"]) {
