                ...
                {!query.isLoading ? (
                    <div>
                        <div className="mx-auto grid max-w-7xl gap-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                            {promptItems.map((item) => (
                                <PromptCard
                                    ...
                                    onCopy={() => copyText(item.prompt, t("prompts.copySuccess"))}
                                    extraAction={
                        ...
                        <div className="mx-auto mt-6 max-w-7xl text-center text-xs text-stone-500 dark:text-stone-400">
                            {query.isFetchingNextPage ? t("prompts.loadingMore") : query.hasNextPage ? t("prompts.scrollMore") : promptItems.length > 0 ? t("prompts.end") : null}
                        </div>
                    </div>
                ) : null}
            </main>

            <PromptDetailDialog prompt={selectedPrompt} onClose={() => setSelectedPrompt(null)} onCopy={(prompt) => copyText(prompt, t("prompts.copySuccess"))} onSaveAsset={savePromptAsset} />
        </div>
    );
}
