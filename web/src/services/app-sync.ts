...
export type AppSyncStage = "waiting-local" | "complete" | "read-remote" | "read-local" | "download-missing" | "apply-merged" | "upload-new" | "upload-manifest" | "done" | "failed" | "scan-missing" | "media-complete" | "download-media" | "scan-local" | "no-upload" | "upload-media";
...
export async function syncAppDataToWebdav(config: WebdavSyncConfig, onProgress?: AppSyncProgress): Promise<AppSyncResult> {
    ...
    emitProgress(onProgress, { stage: "complete", status: "success" });
    return result;
}
...
async function syncDomain<T>(config: WebdavSyncConfig, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    try {
        emitProgress(onProgress, { domain: options.key, stage: "read-remote", status: "active" });
        ...
        emitProgress(onProgress, { domain: options.key, stage: "done", current: 1, total: 1, status: "success" });
        ...
        emitProgress(onProgress, { domain: options.key, stage: "failed", detail: error instanceof Error && !(error instanceof AppError) ? error.message : undefined, status: "exception" });
        throw error;
    }
}
...
async function readDomainManifest<T>(config: WebdavSyncConfig, domain: DomainKey, emptyData: T): Promise<DomainManifest<T> | null> {
    ...
    if (data.app !== "infinite-canvas" || data.domain !== domain) throw new AppError("error.sync.invalidManifest", { domain });
    return {
        app: "infinite-canvas",
        version: 1,
        domain,
