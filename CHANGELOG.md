...
+ [优化] 首页改为本地数据驱动的创作启动台，集中提供工作台入口、最近画布和最近素材。
+ [优化] 完成 Web 应用全局中英文切换，覆盖 Canvas、Agent 日志、WebDAV 进度、生成记录和服务错误，并保持 AI prompt/tool protocol 不变。
+ [修复] 生成记录、同步进度和画布错误不再持久化中文状态；旧数据仍可兼容读取。
+ [优化] Dream 内置视频模型按首尾帧与全能参考展示参数，并接入图片、音频和视频分片素材上传。
