# 画布库项目管理升级：最终实施计划草案（Architect ITERATE 修订版）

> 状态：Planner 修订草案，待主代理继续执行 Architect → Critic 顺序审核。  
> 执行门禁：用户批准最终 `.omx/plans/` 计划前，不修改业务代码。  
> 已吸收反馈：B+ 独立同步域、tombstone 权威删除、显式 store 状态、remove-wins、版本化封面、运行时 archive/manifest 解析、URL allowlist、无 folder order 文档、幂等 normalize 与真实验证命令。
...
- 当前草案必须重新经过 Architect → Critic。
- Critic 非 `APPROVE` 时返回 Planner 修订并重走完整审核。
- 最终计划写入 `.omx/plans/` 且用户明确批准后，才允许修改业务代码。
